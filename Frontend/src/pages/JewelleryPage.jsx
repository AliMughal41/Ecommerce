...
import { Heart, Star, Truck, Shield, Lock, ShoppingCart, Diamond, Loader2, ChevronDown } from 'lucide-react';
...
export default function JewelleryPage({ wishlist, setWishlist }) {
  const { products: allProducts, loading } = useProducts();
  const [products, setProducts] = useState([]);
  const [categories, setCategories] = useState([]);
  const [activeCategory, setActiveCategory] = usePersistentState('jewellery_category', 'All');
  const [sortBy, setSortBy] = usePersistentState('jewellery_sort', 'Newest First');
  const { showAlert } = useAlert();
  const [currentPage, setCurrentPage] = usePersistentState('jewellery_page', 1);
  const [itemsPerPage, setItemsPerPage] = usePersistentState('jewellery_items_per_page', 12);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const superCat = superCatRes.data.superCategories?.find(s => s.name.toLowerCase() === 'jewellery');
        if (superCat) {
          const jewelleryCategories = (categoriesRes.data.categories || [])
            .filter(c => c.superCategory?._id === superCat._id)
            .map(c => c.name);
          setProducts(allProducts.filter(p => jewelleryCategories.includes(p.category)));
        } else {
  ...
  const subCategoryNames = categories
    .filter(c => c.superCategory?.name?.toLowerCase() === 'jewellery')
    .map(c => c.name);
      ...
      <section className="position-relative overflow-hidden hero-section" style={{ height: '100vh', minHeight: '600px', maxWidth: '1920px', margin: '0 auto', background: '#0a0a0a' }}>
        <div className="position-absolute top-0 start-0 w-100 h-100" style={{ zIndex: 0 }}>
          <img src="/images/hero.png" alt="Hero" className="w-100 h-100"
            style={{ objectFit: 'cover', objectPosition: '70% center', opacity: 1, filter: 'brightness(0.85) contrast(1.05) saturate(1.15)' }} />
        </div>
          ...
          <div className="row h-100 align-items-center" style={{ minHeight: 'calc(80vh - 60px)' }}>
            <div className="col-lg-6 d-flex flex-column justify-content-center">
              ...
              <h1 className="fw-bold mb-2" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(48px, 7vw, 84px)', lineHeight: '1', letterSpacing: '0.5px', color: '#fff' }}>
                <span className="d-block">Timeless</span>
                <span className="d-block" style={{ color: '#d4af37' }}>Elegance</span>
              </h1>
              <p className="mb-4" style={{ fontSize: 'clamp(15px, 1.1vw, 18px)', color: 'rgba(255,255,255,0.7)', maxWidth: '520px', lineHeight: '1.8', fontFamily: "'Inter', sans-serif" }}>
                Discover our exquisite jewellery collection — from dazzling rings to statement necklaces. Each piece is crafted to add a touch of luxury to every moment.
              </p>
              <div>
              ...
              <div className="d-flex gap-5 mt-4 pt-3" style={{ borderTop: '1px solid rgba(212,175,55,0.15)' }}>
                ...
                <div>
                  <div style={{ fontSize: '12px', color: '#d4af37', letterSpacing: '1px', fontWeight: '600' }}>DIAMOND</div>
                  <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '11px', letterSpacing: '0.5px' }}>COLLECTIONS</div>
                </div>
                <div>
      ...
      <div className="container-fluid px-3 px-md-4 py-4">
          ...
          <div className="d-flex gap-3 flex-wrap align-items-center">
            ...
            <button onClick={() => navigate('/bags')}
              ...
              style={{
                ...
                border: '1px solid #3d3020', borderRadius: '3px', padding: '8px 24px',
                transition: 'all 0.2s'
              }}
        ...
        {!loading && sorted.length === 0 && (
          <div className="text-center py-5">
            <p className="text-secondary" style={{ fontSize: '16px' }}>No jewellery found for selected filters.</p>
          </div>
        )}
